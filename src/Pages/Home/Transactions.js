...
export default function Transactions({ transaction, setReload, reload }) {
	...
	return (
		<div>
			...
			{transaction.type === "credit" ? <Value credit>{valueFormated}</Value> : <Value>{valueFormated}</Value>}
			<DeleteTransaction transactionId={transaction._id} setReload={setReload} reload={reload} />
		</div>
	);
}
